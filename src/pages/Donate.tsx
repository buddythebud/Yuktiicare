...
import { submitSupport, getRecentDonors, getDonationImpactStats, SupportFormData, RecentDonor } from "@/services/donationService";
import { useAppContext } from "@/context/AppContext";

export default function Donate() {
  const { isAuthenticated, user } = useAppContext();
  
  const [supportAmount, setSupportAmount] = useState<string>("25");
  ...
  const handleSupportSelect = (amount: string) => {
  ...
  const handleSupportSubmit = async () => {
    ...
    try {
      const response = await submitSupport(formData);
        ...
        <section className="section-container">
          ...
          {thankYouVisible ? (
            ...
            >
              ...
              <Button onClick={() => setThankYouVisible(false)}>Support Again</Button>
            </motion.div>
          ) : (
